...
const GCS_BUCKET_NAME = 'cloudprice-comparator.firebasestorage.app';
...
export const fetchPricingData = async (
  ...
  // Ensure that instanceId and pricingModelValue do not contain characters that could be problematic in a URL path.
  // For GCS, instance IDs and pricing model values are usually simple strings, but it's good practice.
  ...
  try {
    ...
    console.error(`[Pricing] Catch block: Error during fetch for ${provider} ${instanceId} in ${regionId} (${pricingModelValue}) from GCS:`);
